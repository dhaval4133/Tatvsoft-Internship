...
export class AddEditMissionThemeComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    console.log("🧪 Form Submitted", this.missionThemeForm.value); //onsubmit console
  ...
  insertData(value: any) {
    const missionThemeSubscribe = this._service
      .addMissionTheme(value)
      .subscribe(
        (data: any) => {
          console.log('📦 Payload to backend:', value); //console
  ...
  updateData(value: any) {
    value.status = value.status === true || value.status === 'active' ? true : false; //value
    const updateMissionThemeSubscribe = this._service
      .updateMissionTheme(value)
      .subscribe(
        (data: any) => {
          if (data.result == 1) {
            this._toast.success({
