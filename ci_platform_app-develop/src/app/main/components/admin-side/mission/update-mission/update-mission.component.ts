...
export class UpdateMissionComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.editMissionForm = this._fb.group({
      id: [this.missionId ?? ''], // ✅ assign missionId here
  ...
  getCountryList() {
    const countryListSubscription = this._commonService
      .countryList()
      .subscribe((data: any) => {
        if (data.result == 1) {
  ...
  getCityList(countryId: any) {
    ...
    const cityListSubscription = this._commonService
      .cityList(countryId)
      .subscribe((data: any) => {
        if (data.result == 1) {
  ...
  getMissionSkillList() {
    const getMissionSkillListSubscription = this._service
      .getMissionSkillList()
      .subscribe(
        (data: any) => {
          if (data.result == 1) {
  ...
  getMissionThemeList() {
    const getMissionThemeListSubscription = this._service
      .getMissionThemeList()
      .subscribe(
        (data: any) => {
          if (data.result == 1) {
  ...
  fetchDetail(id: any) {
    const fetchDetailSubscription = this._service
      .missionDetailById(id)
      .subscribe((data: any) => {
        this.editData = data.data;
        const startDateformat = this._datePipe.transform(
          this.editData.startDate,
          'yyyy-MM-dd'
        );
        this.editData.startDate = startDateformat;
        const endDateformat = this._datePipe.transform(
          this.editData.endDate,
          'yyyy-MM-dd'
        );
        this.editData.endDate = endDateformat;
        const registrationDeadLineDateformat = this._datePipe.transform(
        ...
        if (this.editData.missionImages) {
          ...
          for (const photo of this.imageUrl) {
            this.imageListArray.push(
              this._service.imageUrl + '/' + photo.replaceAll('\\', '/')
            );
          }
        }
        this.unsubscribe.push(cityListSubscription);
      ...
      if (this.isFileUpload) {
        await this._commonService
          ...
          .then(
            ...
            (err) =>
              this._toast.error({ detail: 'ERROR', summary: err.error.message })
          );
      }
      if (this.isFileUpload) {
